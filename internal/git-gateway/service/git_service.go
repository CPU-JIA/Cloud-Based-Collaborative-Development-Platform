...
func NewGitService(repo repository.GitRepository, logger *zap.Logger, gitRoot string, cfg *config.Config) GitService {
	return &gitService{
		repo:    repo,
		logger:  logger,
		gitRoot: gitRoot,
...
func (s *gitService) UpdateRepository(ctx context.Context, id uuid.UUID, req *models.UpdateRepositoryRequest) (*models.Repository, error) {
	...
	if req.DefaultBranch != nil {
		...
		// 执行git symbolic-ref命令更新默认分支
		cmd := exec.Command("git", "symbolic-ref", "HEAD", "refs/heads/"+*req.DefaultBranch)
		cmd.Dir = repo.GitPath
		if err := cmd.Run(); err != nil {
			s.logger.Error("Failed to update default branch in git",
				zap.String("repository_id", id.String()),
				zap.String("new_branch", *req.DefaultBranch),
				zap.Error(err))
			// 继续执行，不要因为Git操作失败而阻止数据库更新
		}
	}
...
func (s *gitService) DeleteRepository(ctx context.Context, id uuid.UUID) error {
	...
	// 异步删除物理Git仓库文件（带重试机制）
	if s.config.Git.AsyncDeleteEnabled {
		go s.asyncDeleteRepository(repo)
	} else {
		// 同步删除
...
// asyncDeleteRepository 异步删除仓库文件，带重试机制
func (s *gitService) asyncDeleteRepository(repo *models.Repository) {
	maxAttempts := s.config.Git.DeleteRetryAttempts
	if maxAttempts <= 0 {
		maxAttempts = 3
	}

	retryDelay := s.config.Git.DeleteRetryDelay
	if retryDelay == 0 {
		retryDelay = 5 * time.Second
	}

	for attempt := 1; attempt <= maxAttempts; attempt++ {
		s.logger.Debug("Attempting to delete repository files",
			zap.String("repository_id", repo.ID.String()),
			zap.String("git_path", repo.GitPath),
			zap.Int("attempt", attempt))

		err := os.RemoveAll(repo.GitPath)
		if err == nil {
			s.logger.Info("Successfully deleted repository files",
				zap.String("repository_id", repo.ID.String()),
				zap.String("git_path", repo.GitPath),
		...
		// 如果是权限错误，尝试修改权限后再删除
		if os.IsPermission(err) && attempt < maxAttempts {
			s.logger.Warn("Permission denied, attempting to change permissions",
				zap.String("git_path", repo.GitPath))

			// 尝试修改权限
			if chmodErr := os.Chmod(repo.GitPath, 0755); chmodErr == nil {
				// 立即重试删除
				if retryErr := os.RemoveAll(repo.GitPath); retryErr == nil {
					s.logger.Info("Successfully deleted repository files after chmod",
						zap.String("repository_id", repo.ID.String()))
					return
				}
			}
		...
		s.logger.Error("Failed to delete repository files",
			zap.String("repository_id", repo.ID.String()),
			zap.String("git_path", repo.GitPath),
			zap.Int("attempt", attempt),
			zap.Error(err))
	...
	// 所有重试都失败，记录最终错误
	s.logger.Error("Failed to delete repository files after all retries",
		zap.String("repository_id", repo.ID.String()),
		zap.String("git_path", repo.GitPath),
...
func (s *gitService) generateCloneURL(projectID uuid.UUID, repoName string) string {
	baseURL := s.config.Git.BaseURL
	if baseURL == "" {
		baseURL = "https://git.example.com"
	}
	return fmt.Sprintf("%s/%s/%s.git", baseURL, projectID.String(), repoName)
}

// generateSSHURL 生成SSH URL
func (s *gitService) generateSSHURL(projectID uuid.UUID, repoName string) string {
	sshHost := s.config.Git.SSHHost
	if sshHost == "" {
		sshHost = "git.example.com"
	}

	// 如果SSH端口不是标准的22端口，需要包含端口信息
	if s.config.Git.SSHPort != 22 && s.config.Git.SSHPort > 0 {
		return fmt.Sprintf("ssh://git@%s:%d/%s/%s.git", sshHost, s.config.Git.SSHPort, projectID.String(), repoName)
	}

	return fmt.Sprintf("git@%s:%s/%s.git", sshHost, projectID.String(), repoName)
}
...
func (s *gitService) initGitRepository(repoPath, defaultBranch string, initReadme bool) error {
	...
	// 如果需要初始化README，创建初始提交
	if initReadme {
		if err := s.createInitialCommit(repoPath, defaultBranch); err != nil {
			s.logger.Warn("Failed to create initial commit with README",
...
// createInitialCommit 创建包含README的初始提交
func (s *gitService) createInitialCommit(bareRepoPath, defaultBranch string) error {
	// 创建临时工作目录
	...
	// 克隆裸仓库到临时目录
	cmd := exec.Command("git", "clone", bareRepoPath, tempDir)
	output, err := cmd.CombinedOutput()
	if err != nil {
		// 对于空仓库，克隆会返回警告但不是错误
		if !strings.Contains(string(output), "warning: You appear to have cloned an empty repository") {
	...
	readmeContent := fmt.Sprintf(`# Project Repository

This repository was created on %s.

## Getting Started

Please update this README file with your project information.
...
`, time.Now().Format("2006-01-02"))
	...
	gitConfig := [][]string{
		...
		{"config", "user.email", "system@cloudplatform.local"},
	}
	...
	// 创建提交
	cmd = exec.Command("git", "commit", "-m", "Initial commit with README")
	cmd.Dir = tempDir
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("failed to commit: %w", err)
	}

	// 如果默认分支不是master，创建并切换到指定分支
	if defaultBranch != "master" && defaultBranch != "" {
		// 创建新分支
		cmd = exec.Command("git", "checkout", "-b", defaultBranch)
		cmd.Dir = tempDir
		if err := cmd.Run(); err != nil {
			return fmt.Errorf("failed to create branch %s: %w", defaultBranch, err)
		}
	}
...
func (s *gitService) getGitDirectoryContent(repoPath, branch, dirPath string) ([]models.FileInfo, error) {
	...
	for _, line := range lines {
		...
		// 解析文件大小（使用-l参数后，blob类型会有大小信息）
		if parts[1] == "blob" && len(parts) >= 5 {
...
func (s *gitService) calculateLineDiff(oldContent, newContent []byte) (added int, deleted int) {
	...
	// 使用简单的LCS（最长公共子序列）算法计算更精确的diff
	return s.calculatePreciseLineDiff(oldLines, newLines)
}

// calculatePreciseLineDiff 使用更精确的算法计算行差异
func (s *gitService) calculatePreciseLineDiff(oldLines, newLines []string) (added int, deleted int) {
	// 创建行内容的映射，用于快速查找
